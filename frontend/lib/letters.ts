...
export async function listLetters(code: string): Promise<Letter[]> {
  const res = await fetch(`/api/letters?code=${encodeURIComponent(code)}`);
  if (!res.ok) return [];
...
export async function getLetter(code: string, id: string): Promise<Letter | null> {
  const res = await fetch(
    `/api/letters/${encodeURIComponent(id)}?code=${encodeURIComponent(code)}`,
  );
  if (!res.ok) return null;
  const data = (await res.json()) as { ok: boolean; letter?: Letter };
  return data.letter ?? null;
...
export async function whisperOnLetter(
  code: string,
  id: string,
): Promise<string> {
  const res = await fetch(
    `/api/letters/${encodeURIComponent(id)}/whisper?code=${encodeURIComponent(code)}`,
    { method: "POST" },
  );
  if (!res.ok) return "";
